...
def get_data(code, start="2022-01-01", end="2024-09-30"):
    ...
    df['openinterest'] = 0  # 添加空头持仓量列
    df.sort_index(ascending=True, inplace=True)  # 确保按时间升序排列
...
if __name__ == "__main__":
    ...
    dataframe = get_data("600133", start=start.strftime('%Y-%m-%d'), end=end.strftime('%Y-%m-%d'))
